...
for j=1:no_layers
    ...
    beta(:,j)   = 2.*pi.*refractive_index(:,j).*cos(thetam(:,j))./lambda;
end
...
A=ones(length_of_run,1); %Incident right going E field component
...
for r=2:no_layers-1
    ...
    mat_n=mat_n+(real(refractive_index(1,r))).^2.*x_current_points;
