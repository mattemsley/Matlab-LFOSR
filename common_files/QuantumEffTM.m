function [QE,error]=QuantumEffTM(lambda,refractive_index,thickness,theta,active_layer)

if theta~=0 & 1==1
    temp_no_layers=length(thickness);
    temp_refractive_index=refractive_index;
    temp_thickness=thickness;

    clear thickness
    clear refractive_index

    for q=1:active_layer-1
        refractive_index(:,q)=temp_refractive_index(:,q);
        thickness(q)=temp_thickness(q);
    end

    refractive_index(:,q+1)=ones(size(lambda));
    thickness(q+1)=0;
    refractive_index(:,q+2)=temp_refractive_index(:,active_layer);
    thickness(q+2)=temp_thickness(active_layer);
    refractive_index(:,q+3)=ones(size(lambda));
    thickness(q+3)=0;

    if active_layer<temp_no_layers
        for q=active_layer+1:temp_no_layers
            refractive_index(:,q+2)=temp_refractive_index(:,q);
            thickness(q+2)=temp_thickness(q);
        end
    end
...
if no_layers>2  %Matrix formulation not used for two layer structure
    for j=2:no_layers-1  %middle layer matrix
        ...
        TEMP_21(:,1)=z_21(:,j);
        TEMP_22(:,1)=z_22(:,j);
        %figure,plot(lambda,eta(:,j),'d',lambda,delta(:,j),'o'),title(num2str(j))
    end
end
%D1 Inverse
